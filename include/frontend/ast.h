...
extern int lev;           //???
//存放op字段的字符串数组，便于输出IR。
extern char IR_op_strs[75][32];
extern struct node *out_ast;
extern int float_flag;
extern int opti_flag;
extern int DeFlag;
...
struct opn
{
    char kind;         //标识变量的类型；V,P，T，A，F,C-,L,H,S;
...
public:
    ...
    string cont; //指令条件码；
    int paraNo = -1;
    int whileCFlag; //循环条件块头指令标志；
    codenode *gotoTCode, *gotoFCode;
    codenode()
    { //初始化为空语句；
        this->cal_type = 'i';
        this->op = IR_VOID;
